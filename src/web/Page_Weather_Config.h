...
void send_weather_config_html()
{
  ...
  if (server.args() > 0) // Save Settings
  {
    ...
    {
      if (server.argName(i) == "OpenWeatherKey")
        config.OpenWeatherKey = urldecode(server.arg(i));
      if (server.argName(i) == "OpenWeatherCityId")
        config.OpenWeatherCityId = urldecode(server.arg(i));
    }
    WriteConfig();
  }
...
{
  if (Oweather.deg >= 345 || Oweather.deg <= 22)
    return "Північний";
  if (Oweather.deg >= 23 && Oweather.deg <= 68)
    return "Північно-Східний";
  if (Oweather.deg >= 69 && Oweather.deg <= 114)
    return "Східний";
  if (Oweather.deg >= 115 && Oweather.deg <= 160)
    return "Південно-східний";
  if (Oweather.deg >= 161 && Oweather.deg <= 206)
    return "Південний";
  if (Oweather.deg >= 207 && Oweather.deg <= 252)
    return "Південно-західний";
  if (Oweather.deg >= 253 && Oweather.deg <= 298)
    return "Західний";
  if (Oweather.deg >= 299 && Oweather.deg <= 344)

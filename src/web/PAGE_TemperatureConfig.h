...
void save_Temperature_configuration_html()
{
  if (server.args() > 0) // Save Settings
  {
    ...
    for (uint8_t i = 0; i < server.args(); i++)
    {
      ...
      {
        if (server.arg(i) == "auto")
          config.relayBoiler = 0;
        if (server.arg(i) == "ON")
          config.relayBoiler = 2;
        if (server.arg(i) == "OFF")
          config.relayBoiler = 1;
      }
      if (server.argName(i) == "relay_smallroom")
      {
        ...
        if (server.arg(i) == "ON")
          config.relaySmallroom = 2;
        if (server.arg(i) == "OFF")
          config.relaySmallroom = 1;
      }
      if (server.argName(i) == "relay_bedroom")
      {
        ...
        if (server.arg(i) == "ON")
          config.relayBedroom = 2;
        if (server.arg(i) == "OFF")
          config.relayBedroom = 1;
      }
      if (server.argName(i) == "night_temp")
        config.nightTemperature = server.arg(i).toFloat();
      if (server.argName(i) == "night_giterezis")
        config.nightGiterezis = server.arg(i).toFloat();
      if (server.argName(i) == "time_night_start")
        config.timeNightStart = timeToString(server.arg(i));
      if (server.argName(i) == "time_night_stop")
        config.timeNightStop = timeToString(server.arg(i));

      if (server.argName(i) == "morning_temp")
        config.morningTemperature = server.arg(i).toFloat();
      if (server.argName(i) == "morning_giterezis")
        config.morningGiterezis = server.arg(i).toFloat();
      if (server.argName(i) == "time_morning_start")
        config.timeMorningStart = timeToString(server.arg(i));
      if (server.argName(i) == "time_morning_stop")
        config.timeMorningStop = timeToString(server.arg(i));

      if (server.argName(i) == "day_temp")
        config.dayTemperature = server.arg(i).toFloat();
      if (server.argName(i) == "day_giterezis")
        config.dayGiterezis = server.arg(i).toFloat();
      if (server.argName(i) == "time_day_start")
        config.timeDayStart = timeToString(server.arg(i));
      if (server.argName(i) == "time_day_stop")
        config.timeDayStop = timeToString(server.arg(i));

      if (server.argName(i) == "evening_temp")
        config.eveningTemperature = server.arg(i).toFloat();
      if (server.argName(i) == "evening_giterezis")
        config.eveningGiterezis = server.arg(i).toFloat();
      if (server.argName(i) == "time_evening_start")
        config.timeEveningStart = timeToString(server.arg(i));
      if (server.argName(i) == "time_evening_stop")
        config.timeEveningStop = timeToString(server.arg(i));

      if (server.argName(i) == "sunday_temp")
        config.sundayTemperature = server.arg(i).toFloat();
      if (server.argName(i) == "sunday_giterezis")
        config.sundayGiterezis = server.arg(i).toFloat();
      if (server.argName(i) == "time_sunday_start")
        config.timeSundayStart = timeToString(server.arg(i));
      if (server.argName(i) == "time_sunday_stop")
        config.timeSundayStop = timeToString(server.arg(i));

      if (server.argName(i) == "fan_temp")
        config.fanTemperature = server.arg(i).toFloat();
      if (server.argName(i) == "fan_giterezis")
        config.fanGiterezis = server.arg(i).toFloat();
      if (server.argName(i) == "time_fan_start")
        config.timeFanStart = timeToString(server.arg(i));
      if (server.argName(i) == "time_fan_stop")
        config.timeFanStop = timeToString(server.arg(i));
    }
